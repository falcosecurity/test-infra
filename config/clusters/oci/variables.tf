...
variable "config_file_profile" {
  description = "OCI CLI config profile used for local Terraform operations."
  type        = string
...
variable "prow_logs_bucket_name" {
  description = "Prow log and artifact bucket name. Must match the bootstrap IAM policy."
  type        = string
...
variable "cluster_autoscaler_enabled" {
  description = "Install the Cluster Autoscaler add-on. Its workload identity policy must already exist in bootstrap."
  type        = bool
  default     = true
  nullable    = false
}
