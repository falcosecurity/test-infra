locals {
  # Cluster add-ons run on the on-demand platform node pool; every worker pool is tainted.
...
resource "oci_containerengine_addon" "cluster_autoscaler" {
  count = var.cluster_autoscaler_enabled && length(local.autoscaled_node_pools) > 0 ? 1 : 0
...
resource "oci_containerengine_addon" "core_dns" {
  ...
  depends_on = [oci_containerengine_addon.metrics_server]
